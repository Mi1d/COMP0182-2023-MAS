...
def create_agents(yaml_file):
    ...
    for agent in agent_yaml_params["agents"]:
        ...
        if first == True:
        	box_id = p.loadURDF("data/turtlebot1.urdf", start_position, start_orientation, globalScaling=2.1)
        	first = False
        else:
            box_id = p.loadURDF("data/turtlebot2.urdf", start_position, start_orientation, globalScaling=2.1)
...
def navigation(agent, goal, schedule):
    ...
    while(not checkPosWithBias(basePos[0], goal, dis_th)):
        ...
        if agent == 75:
            k1, k2, A = 30, 8, 10
            linear = k1 * ((distance * 20) ** (1/8)) * (((2 / math.pi) * abs(theta) - 1) ** 6)
            angular = k2 * theta
        else:
            k1, k2, A = 20, 10, 20
            linear = k1 * math.cos(theta) * distance
            angular = k2 * theta
        ...
        # time.sleep(0.001)
    p.setJointMotorControl2(agent, 0, p.VELOCITY_CONTROL, targetVelocity=-100, force=100)
    p.setJointMotorControl2(agent, 1, p.VELOCITY_CONTROL, targetVelocity=-100, force=100)
    time.sleep(0.1)
